@extends('layouts.app')
@section('title','Show '.ucfirst(config('settings.receipts_label_singular')))
@section('content')
    <section class="content-header">
        <h1>
            {{ucfirst(config('settings.receipts_label_singular'))}}
            <span class="pull-right">
            <a href="{{ route('receipts.index') }}" class="btn btn-default">
                <i class="fa fa-chevron-left" aria-hidden="true"></i> Back
            </a>
              <button onclick="printInvoice()" class="btn btn-default btn-sm">
                <i class="fa fa-print"></i> Print
            </button>
             <a href="{{ route('receipts.pdf', $invoice->id) }}" class="btn btn-primary btn-sm">
        ...
        <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
                <li class="active"><a href="#invoice" data-toggle="tab"
                                      aria-expanded="true">{{ucfirst(config('settings.receipts_label_singular'))}}</a>
                </li>
